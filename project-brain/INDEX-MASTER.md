...
<!-- type: INDEX -->
<!-- updated: 2026-02-22 -->
<!-- total-files: 67 (51 in INDEX-MASTER + 16 in INDEX-claude-code) -->
...
L053|MCP,agent-mail,multi-agent,IPC,WSL,file-locking,POSIX,Windows,shared-workspace,operational,deployment|→S004,L026,L041,L042,L029,S001|←∅|MCP Agent Mail deployed on WSL — Windows SoftFileLock fails under concurrent multi-agent writes ("device or resource busy"), POSIX locking on WSL Ubuntu 24.04 resolves. Server at http://127.0.0.1:8766 (accessible from Windows). Second problem: project-scoped messaging derives key from working directory — agents in different dirs can't message each other. Solution: shared workspace (C:\shared-workspace) with .mcp.json. Trailing slash on /mcp/ URL required (silent failure without). Alias-based path mapping attempted but reverted (cross-platform path resolution). Stale .archive.lock files after crashes need manual cleanup.|d:WSL>Windows for file-locked MCP servers; shared workspace>path aliasing for project scoping; centralized broker (HTTP) vs serverless (S004 AMP) tradeoff|i:ensure_project(human_key)→register_agent(project_key,program,model,task)→send_message/fetch_inbox. Web UI at /mail|!server requires manual WSL start; stale locks after crashes; no auto-reconnect
