...
<!-- type: INDEX -->
<!-- updated: 2026-02-20 -->
<!-- total-files: 64 (48 in INDEX-MASTER + 16 in INDEX-claude-code) -->
...
## How to Use
1. Check SESSION-HANDOFF.md first — previous session may have left unfinished work
2. Scan entries below — each should tell you whether to open the file or skip it
3. Sub-indexes in `indexes/` for large clusters. Format spec: LEARN-046
4. Use brain.py or MCP search_brain OUTSIDE context when possible
5. Write SESSION-HANDOFF.md before ending any session
...
@SUB:claude-code|indexes/INDEX-claude-code.md|16|L004,L005,L006,L007,L008,L009,L010,L013,L014,L015,L016,L017,L018,L019,L039,G003|Claude Code internals: memory,skills,hooks,subagents,MCP,agent-SDK,plugins,costs,workflows,architecture,brain integration. Load when working on Claude Code integration. Squeeze point active (31% of brain).
...
S005|architecture,brain-v2,three-space,link-index,pipeline,reweave,schema-enforcement,search,hypertext,knowledge-management,migration|→S000,L048,L049,L050,L031,L032,S003,L033,L030,L040,L034,L044,C001,R005,L046,L024,L009,L023,R001|←∅|Brain v2 architecture. Synthesizes 63 brain files into next-gen design. 6 problems: link-blind search, no backward pass, hub concentration, mixed spaces, manual deposit bottleneck, monolithic index. 6 solutions: (1) three-space separation (identity/knowledge/ops — L050), (2) three-layer index (fat index + link index with typed relationships + cluster indexes), (3) 5-phase pipeline (Deposit→Connect→Reweave→Verify→Synthesize) with fresh subagent per phase, (4) schema enforcement via PostToolUse hooks, (5) three-case search (within-file/cross-link/cross-chain — adapted from L048/L049 hypertext decomposition), (6) auto-deposit SessionEnd hook→QUEUE.md. Core insight: brain should be hypertext with search engine, not document store with index. Incremental migration: 5-7 sessions across 6 phases.|d:three-space invariant; link index separate from fat index; reweave bounded (1-hop, max 5 files); BFS not range query at current scale; relationship-typed links (extends/validates/contradicts/etc); schema blocks not warns; SPECs in identity/ (open Q)|i:search_brain(query,space), search_linked(source,target,relationship), search_path(start,end,max_hops). Pipeline: /brain-deposit→/brain-reweave→/brain-verify→/brain-synthesize|!5 open Qs (SPEC placement, link index format, reweave depth, IDENTITY.md vs CLAUDE.md, schema strictness). Migration churn risk. Reweave scope creep. Complexity budget vs R005.
