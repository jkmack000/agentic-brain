...
<!-- created: 2026-02-20 -->
<!-- source: Synthesis of 63 brain files. Key inputs: S000 (current arch), L048/L049 (hypertext indexing), L050 (Ars Contexta), L031 (zettelkasten), L032/S003 (quorum sensing), L030 (BM25), L040 (memory strategies), L034 (capture gap), L033 (topology), L044 (context multiplier), C001 (MCP server), R005 (user prime directive). -->
<!-- links: S000, L048, L049, L050, L031, L032, S003, L033, L030, L040, L034, L044, C001, R005, L046, L024, L009, L023, R001 -->
...
### Index Architecture: Three Layers

Inspired by the hypertext index's dual forward/reverse design (L048) and our fat index format (L046):
...
New. A dedicated structure mapping edges between files:
```
# LINK-INDEX.md
# Format: source|target|type|context
L048|L049|extends|"L049 algorithm operates on L048's data structure"
L048|L044|validates|"dual indexing analogous to forward links + backlinks"
L048|S000|implements|"hypertext index is a type of fat index"
L031|L032|inspires|"zettelkasten principles led to quorum sensing framework"
```

Relationship types: `extends`, `validates`, `contradicts`, `implements`, `inspires`, `supersedes`, `corroborates`.

This enables case (ii) search from L049: "find files about X that link to files about Y" as a single indexed operation. The link index is the 2D range query analog — edges as queryable points.
...
**Scaling note**: At ~60 files with avg ~3.5 outlinks (L033), the graph is sparse enough that BFS with depth limit is practical. We don't need the full orthogonal range query machinery (L048) until hundreds of files. Reassess at 100+ files (L030 roadmap).
...
| 2 | Link index format: markdown table vs. compressed-v1 pipe format? | Tables more readable; pipes more token-efficient |
