...
- **Start:** Check SESSION-HANDOFF.md, then scan INDEX-MASTER.md for relevant prior work
- **During:** Follow the Workflow below
- **End:** Write SESSION-HANDOFF.md + append LOG-002 timeline entry
...
All brain files live in `project-brain/`. File types:

| Type | Purpose | Directory |
|------|---------|-----------|
| SPEC | Design decisions, architecture | `specs/` |
| CODE | Implementation documentation | `code/` |
| RULE | Business rules, constraints | `rules/` |
| LEARN | Discovered knowledge, insights | `learnings/` |
| LOG | Decision rationale, timeline | `logs/` |
| RESET | Pre-built context packages | `reset-files/` |
